...
include "../koneksi.php";
$lihat=mysqli_query($koneksi,"select * from gambar order by id DESC");
...
$i++;
  ...
  <tr>
	...
	?></td>
    <td width="17%" align="center" ><!--<a href="index.php?menu=editlow.php&data=<?=$low[0];?>" class="ngedit"><button type="button" class="btn btn-info">Edit</button></a>--></td>
    <td width="15%" align="center" ><a href="index.php?menu=showbanner.php&data=<?=$low[0];?>&aksi=hapus" class="ngedit"><button type="button" class="btn btn-danger">Hapus</button></a></td>
  </tr>
  <?php } ?>
</table>
<?php
...
if($_GET["aksi"]=="hapus"){
 ...
 <div style="border-color:#F00; background-color:#CF9; border-style: dotted; padding-top:10px; padding-left:10px; padding-bottom:10px" > 
    <?php
$lihat=mysqli_query($koneksi,"select * from gambar where id=$_GET[data]");
...
$sql=mysqli_query($koneksi,"delete from gambar where id=$_GET[id]");
if($sql) //header("location:index.php?menu=showbanner.php");
 echo("<meta http-equiv=\"refresh\" content=\"0;url=index.php?menu=inputbanner.php\">");
}
